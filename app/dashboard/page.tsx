                  ...
                  <div style={{ marginTop: '20px' }}>
                    ...
                    {(() => {
                      ...
                      return (<div>
                        ...
                        <h4 style={{ margin: '16px 0 12px 0', color: theme.text, fontSize: '14px' }}>📐 R:R Payout Table</h4>
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '8px', marginBottom: '16px' }}>
                          {rrRatios.map(rr => (<div key={rr} style={{ padding: '12px', background: darkMode ? '#1e293b' : '#f8fafc', borderRadius: '10px', textAlign: 'center' as const, border: '1px solid ' + theme.border }}><div style={{ color: theme.textMuted, fontSize: '10px' }}>Risk ${riskAmount.toFixed(0)}</div><div style={{ color: theme.accent, fontSize: '10px', fontWeight: 700, margin: '4px 0' }}>{rr}R</div><div style={{ color: theme.success, fontSize: '16px', fontWeight: 800 }}>+${(riskAmount * rr).toFixed(0)}</div></div>))}
                        </div>
