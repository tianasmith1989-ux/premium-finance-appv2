...
export default function Dashboard() {
  ...
  const addTransactionToCalendar = (transaction: any) => {
    ...
    // Move to next occurrence if past
    while (nextDate < today) {
      if (transaction.frequency === 'weekly') nextDate.setDate(nextDate.getDate() + 7)
      else if (transaction.frequency === 'fortnightly') nextDate.setDate(nextDate.getDate() + 14)
      else if (transaction.frequency === 'monthly') nextDate.setMonth(nextDate.getMonth() + 1)
      else if (transaction.frequency === 'yearly') nextDate.setFullYear(nextDate.getFullYear() + 1)
      else break
    }
  ...
  const addDebtToCalendar = (debt: any) => {
    ...
    // Add monthly payment to calendar (assume 1st of next month)
    const today = new Date()
                  ...
                  {transactions.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      {transactions.map(t => {
                        ...
                        return (
                          <div key={t.id} style={{ padding: '16px', background: t.type === 'income' ? '#f0fdf4' : '#fef2f2', borderRadius: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                            <div style={{ flex: 1 }}>
                    ...
                    <>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                        {debts.map(debt => {
                          ...
                          return (
                            <div key={debt.id} style={{ padding: '16px', background: '#fef2f2', borderRadius: '12px', border: '2px solid #ef4444', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                              <div style={{ flex: 1 }}>
